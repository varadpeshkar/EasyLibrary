...
class AdminModel
{
    ...
    private static function writeDeleteAndSuspensionInfoToDatabase($userId, $suspensionTime, $delete)
    {
        ...
        $query = $database->prepare("UPDATE admin_users SET user_suspension_timestamp = :user_suspension_timestamp, user_deleted = :user_deleted  WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(
    ...
    private static function resetUserSession($userId)
    {
        ...
        $query = $database->prepare("UPDATE admin_users SET session_id = :session_id  WHERE user_id = :user_id LIMIT 1");
        $query->execute(array(
                ':session_id' => null,
                ':user_id' => $userId
        ));
